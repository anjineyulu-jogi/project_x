...
# --- 🧠 GEMINI 3 FLASH CONFIGURATION (SECURE) ---
# Pulls your new key from Render Env Var
api_key = os.environ.get("GEMINI_API_KEY")
client = genai.Client(api_key=api_key)
MODEL_ID = "gemini-3-flash-preview"  # Corrected model ID
...
Analyze the food data JSON. Output ONLY a valid JSON object:
{
  ...
  "warning_flags": ["List specifically harmful additives or high sugar/salt"],
...
async def analyze_with_gemini(product_data):
    """Sends DNA data to Gemini 3 Flash and cleans the response."""
    try:
        if not api_key:
            return {"narrative": "Error: GEMINI_API_KEY missing in Render."}
